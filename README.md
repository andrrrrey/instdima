...
- **План** — контент-календарь: создание/редактирование/перенос/удаление публикаций,
  загрузка медиа — до 20 фото/видео на публикацию с массовым выбором из медиатеки
  телефона/ПК, выбором обложки и удалением отдельных файлов, рабочие статусы, дедлайны,
  комментарии, история изменений. Состав вкладок, полей и кнопок зависит от роли.
...
### Суперадмин («просмотр от имени»)

Telegram ID суперадминов задаётся в `SUPERADMIN_IDS` (через запятую). Суперадмин:

- заходит всегда — даже если его нет в `ALLOWLIST` (создаётся автоматически как владелец);
- в шапке приложения получает переключатель **«просмотр от имени»** и может открыть
  интерфейс любого участника (владелец / контент-мейкер / заказчик), чтобы проверить, как всё
  выглядит и работает в каждой роли;
- обычные пользователи переключателя не видят и работают только в своей роли.

Технически это настоящая имперсонация: клиент шлёт заголовок `X-View-As: <userId>`, и бэкенд
отдаёт данные и урезает поля ровно как для выбранного пользователя (`server/src/auth/session.js`).
Выбор сохраняется между заходами. Действия в режиме просмотра выполняются от имени выбранного
участника — используйте для проверки, а не для боевого ведения чужих задач.
